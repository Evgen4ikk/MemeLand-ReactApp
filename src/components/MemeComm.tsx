...
const MemeComm: React.FC<MemeCommProps> = ({ memeId }) => {
	...
	const { data: comments } = memeAPI.useFetchCommentsMemeIdQuery(memeId)

	const { data: myProfile } = userAPI.useFetchProfileDataQuery('')

	const [createComments] = memeAPI.useCreateCommentMutation()
	...
	return (
		<div className='pt-4 w-full'>
			<h1 className=' text-xl font-medium text-[#f1f1f1] pb-2'>
				...
				{comments?.length === 1
					? ' комментарий'
					: comments?.length && comments?.length > 1 && comments?.length < 5
					? ' комментария'
					: ' комментариев'}
			</h1>
			...
			<CommItem memeId={memeId} myProfile={myProfile} comments={comments} />
		</div>
	)
}
