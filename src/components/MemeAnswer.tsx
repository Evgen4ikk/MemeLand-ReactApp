...
const MemeAnswer: React.FC<MemeAnswerProps> = ({ memeId, comment }) => {
	...
	const { data: answersById = [] } = memeAPI.useFetchCommentAnswersQuery(comment.id)

	const [createAnswers] = memeAPI.useCreateAnswerMutation()
	...
	const [isAnswerInputClicked, setIsAnswerInputClicked] =
	...
	return (
		<div className='w-full'>
			<div className='flex items-center pt-2'>
				...
				<div>
					<div className='hover:bg-[#3f3f3f] rounded-full ml-2'>
						...
						>
							<span className='text-[#f1f1f1] text-xs font-medium'>
								Ответить
							</span>
						</IconButton>
					</div>
			...
			<div className='w-[700px]'>
				{isAnswerClicked && (
					<form onSubmit={handleCreateAnswer} className='w-full'>
						<div className='flex'>
							<div className='mr-5'>
								<Link to={`/MyProfile`}>
									<Avatar
				...
				<div>
					{answersById.length > 0 && (
						...
						>
							...
							{answersById.length}
							{answersById.length === 1
								? ' ответ'
								: answersById.length &&
								  answersById.length > 1 &&
								  answersById.length < 5
								? ' ответа'
								: ' ответов'}
						</button>
					)}
